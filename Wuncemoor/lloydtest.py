...
def continents_generate(finalarray, border_nodes, non_border_nodes, width, height, num_cells, new_centroids):
    ...
    water_nodes = set()
    continent_nodes = set()
    continents_nodes = set()
    nodes_in_continent = []
    land_mass = math.floor(num_cells*0.08)
    ...
    while looped < loops:
        ...
        while len(continent_centroids) < cells_per_continent[looped]:
            ...
            if len(continent_centroids) < 2:
                ...
                for i in finalarray:
                    ...
                    #Find the centroid of the node that starting town is in
        ...
        for i in finalarray:
            ...
            for centroid in continent_centroids:
                ...
                if i[1] == centroid[0] and i[2] == centroid[1]:
                    if i[0] in (continents_nodes or water_nodes):
                        pass
                    else:
                
                        continent_nodes.add(i[0])
                        continents_nodes.add(i[0])
                    
                    
        # for i in finalarray:
            # for j in finalarray:
                # if i[0] in continent_nodes and i[0] != j[0] and (i[1], i[2]) == ((j[1], j[2]+1) or (j[1], j[2]-1) or (j[1]+1, j[2]) or (j[1]-1, j[2])):
                    # if j[0] in continent_nodes:
                        # pass
                    # else:
                        # water_nodes.add(j[0])
            ...
            # if i[0] in continent_nodes:
                ...
                # for j in finalarray:
                    ...
                    # if j[1] == nearby_right[0] and j[2] == nearby_right[1]:
                        ...
                        # if i[0] != j[0]:
                            # if j[0] in (continent_nodes or continents_nodes):
                                # pass
                            # else:
                                # water_nodes.add(j[0])
    ...
    for i in finalarray:
        ...
        if i[0] in water_nodes:
            ...
            putpixel((i[1], i[2]), (255, 0, 0))
                    
    image.save('Continentstest.png', 'PNG')
    image.show()

    return finalarray, water_nodes, continents_nodes
